            ...
            for batch in response:
                for row in batch.results:
                    records.append(QualityScoreRecord(
                        ...
                        landing_page_exp=row.ad_group_criterion.quality_info.historical_landing_page_quality_score.name
                        if row.ad_group_criterion.quality_info.historical_landing_page_quality_score
                        else "UNSPECIFIED",
                        ad_relevance=row.ad_group_criterion.quality_info.historical_creative_quality_score.name
                        if row.ad_group_criterion.quality_info.historical_creative_quality_score
                        else "UNSPECIFIED",
                        expected_ctr=row.ad_group_criterion.quality_info.historical_search_predicted_ctr.name
                        if row.ad_group_criterion.quality_info.historical_search_predicted_ctr
                        else "UNSPECIFIED",
                        impressions=row.metrics.impressions,
                        cost=row.metrics.cost_micros / 1_000_000,
                        currency=row.customer.currency_code,
