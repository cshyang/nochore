...
A general-purpose agent platform where AI agents replace traditional functions. Agents have reasoning (LLM), skills (domain knowledge), tools (actions via Composio), and policy/guardrails.
...
- **TanStack Start** (React 19, Vite 7) — frontend with `@ai-sdk/react` for chat

### Apps

**`apps/web/`** — TanStack Start frontend. Run with:
```bash
...
- `server/` — Server functions bridging harness to frontend
- `lib/colors.ts` — Design tokens (source of truth: `.impeccable.md`)

### Packages

**`packages/harness/`** — Core harness (365 tests passing). Contains:
- `types/` — Zod contracts (Skill, Action, Policy, Memory, AgentConfig, Run)
- `pipeline/` — 8-step pipeline runner (scope → fetch → analyze → plan → policy → execute → memory)
- `policy/` — Deterministic policy engine with 4 built-in rules
- `skills/` — Skill registry + executor (deterministic or LLM via AI SDK `generateObject`)
- `memory/` — SQLite-backed event log + lessons
- `context/` — ContextAssembler (merges workspace .md files + DB state, shared by pipeline and chat)
- `workspace/` — WorkspaceStore (path-secured .md file access)
- `chat/` — Chat handler (AI SDK `streamText` + domain/workspace tools)
- `connections/` — ConnectionManager interface (Composio wrapper)
- `repositories/` — Run, Approval, ChatSession, Lesson, Event repositories
...
- **Three-Layer Split**: Files own authored context (.md), DB owns operational truth (SQLite), JSONL is optional debug mirror
...
- **Policy gate is ALWAYS deterministic** — no LLM, ever. Strictest rule wins.
- **Agent = directory + DB record** — .md workspace (identity) + SQLite row (operational state)
