...
export const getAgentActivityState = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => jsonSafe(await loadAgentActivityState(data.projectId, data.agentId)));

export const getProjectActivityState = createServerFn({ method: "GET" })
  .inputValidator((input: { projectId: string }) => input)
  .handler(async ({ data }) => jsonSafe(await loadProjectActivityState(data.projectId)));
...
export async function loadAgentActivityState(projectId: string, agentId: string) {
  ...
  const latestActiveRun = activeRuns.sort((left, right) => right.startedAt.getTime() - left.startedAt.getTime())[0] ?? null;
  const timestamps = [
...
export function isActionableApproval(approval: ApprovalRecord): approval is ApprovalRecord & { status: "pending" | "expired" } {
